...
export async function write(websiteName, tag, data) {
  ...
  // Write JSON using the filename declared in registry.json
  const jsonFile = entry.outputFiles.find((f) => f.endsWith('.json')) ?? `${tag}.json`;
  const jsonPath = path.join(outputDir, jsonFile);
  fs.writeFileSync(jsonPath, JSON.stringify(data, null, 2), 'utf8');

  // If registry specifies a .css output file, generate CSS custom properties
  const cssFile = entry.outputFiles.find((f) => f.endsWith('.css'));
  if (cssFile) {
    const cssPath = path.join(outputDir, cssFile);
    const cssContent = buildCssCustomProperties(tag, data);
