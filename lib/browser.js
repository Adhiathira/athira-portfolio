 ...
 * @param {Page} page
 */
export async function warmupPage(page) {
...
export async function dismissCookieBanners(page) {
  // Wait for JS-injected CMPs (e.g. Didomi) to render before scanning.
  // Didomi and similar platforms inject their UI after 'load' fires, so a
  // brief wait prevents the quick-check pass from running before the banner exists.
  await page.waitForTimeout(1500);

  // Specific CMP selectors (high confidence, click without validation)
  const SPECIFIC_SELECTORS = [
    // Didomi (common on European luxury/fashion sites)
