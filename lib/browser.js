...
export async function dismissCookieBanners(page) {
  ...
  if (matchedSelectors.length === 0) {
    ...
    for (const label of acceptLabels) {
      ...
      try {
        ...
        if (visible) {
          console.log(`[browser] Dismissing cookie banner (text fallback): "${label}"`);
    ...
    if (!fallbackClicked) {
      console.log('[browser] No cookie banner detected');
    }
    return;
  }
  ...
  for (const selector of matchedSelectors) {
    try {
      const button = page.locator(selector).first();
      console.log(`[browser] Dismissing cookie banner: ${selector}`);
      ...
      if (navigationOccurred) {
        console.warn(`[browser] Click caused navigation: ${originalUrl} -> ${currentUrl.href}`);
        console.warn('[browser] Recovering by navigating back to original URL');

        try {
          await page.goto(originalUrl, { waitUntil: 'load', timeout: 10000 });
          console.log('[browser] Successfully recovered original page state');
          ...
          console.error(`[browser] Failed to recover original URL: ${err.message}`);
          console.warn('[browser] Continuing with current page state despite recovery failure');
      ...
      if (!stillVisible) {
        console.log('[browser] Cookie banner dismissed successfully');
  ...
  try {
    ...
    await page.waitForTimeout(CLICK_DELAY);
    console.log('[browser] Attempted cookie banner dismissal via Escape key');
  } catch (err) {
    // Ignore errors
  }

  // If we got here, no cookie banner was detected or all dismissals failed
  console.log('[browser] No cookie banner detected or dismissal not needed');
}
