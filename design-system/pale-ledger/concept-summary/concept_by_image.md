...
The hero fires in sequence. The headline — 120.96px, weight 700, -5.184px letter-spacing, 0.9 line-height — launches upward from -66px offset, covering that distance in exactly 0.3s on a cubic ease-in-out, arriving flush into position with the momentum of a controlled drop. The instant the type settles, the cursor begins: a typewriter blink looping at 1s intervals, a steady pulse that keeps the hero kinetically alive long after the entry animation completes. Two-line headline structure generates its own internal tempo — the first line strikes at full #ffffff brightness, the second line follows in #737373 gray, a one-two cadence that reads as foreground thrust then receding echo within a single statement. A sparkle icon marks the leftward anchor of the text block, a fixed point from which the headline radiates rightward across nearly the full container. The single #006cf7 CTA pill below completes the descent, and the whole sequence — text lands, cursor ticks, blue activates — takes less than half a second to reach full operational state.
...
- **Motion is always translational, never rotational** — all entry keyframes use translateY (8–80px range); no spin, no scale for content; motion direction is consistently upward-into-place
