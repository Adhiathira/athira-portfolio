...
The typography system holds its range under deliberate stress. At one end: `interstate-condensed` at 85.5px, uppercase, weight 400 — a compressed letterform released at viewport width, carrying the visual force of a woodblock print. At the other: `interstate` at 14.256px, weight 300, lower-case, nearly inaudible. The ratio is approximately 6:1, and that number does not fully communicate the qualitative leap — this is not a scale variation, it is a register collapse. The condensed face compresses horizontal mass into vertical energy, so when it opens across the full viewport it does not merely read large; it exerts pressure. Body text at 19.008px with 28.512px leading exists in a separate atmospheric band entirely — generous, unhurried, cut loose from the display's urgency. The typographic system is engineered around this gap: the jump from label to headline is not graduated. It is an interruption.
