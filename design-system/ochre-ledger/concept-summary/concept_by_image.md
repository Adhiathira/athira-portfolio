...
Backgrounds do not use strong alternating light/dark transitions. Instead, the interface maintains a **persistent dark narrative** with minor warm-cool shifts as the primary pacing mechanism. This creates atmospheric continuity rather than section-by-section contrast.
...
Every component here carries the material logic of pressed board — these are not arbitrary rectangles but surfaces with a specific weight and grain to them. **Cards** emerge from a slightly warmer composite than the section ground (`#0f2128` vs `#0d1a1f`), the tonal shift narrow enough that the card reads less as contrast and more as density — the way a sheet of card stock lifted from a surface catches a fraction more light. The separation is achieved through **background warmth and spacing**, not outline or shadow: the card simply occupies a distinct material register, like a different paper weight on the same table. `16px` border-radius softens the perimeter, suggesting a fabricated edge — pressed and rounded, not cut.

**FAQ accordion rows** use dark tiles with hairline borders (`#2a3540`) — the minimum threshold of surface separation, the kind of material seam that reads as ruled construction rather than drawn line.

**Buttons (primary/filled):** The terracotta CTA (`#9d5e5b`) uses a layered shadow that gives it tactile lift — the most physically present component on the page, something you could press. `8px` radius.
...
**Pricing card (highlighted):** Warm brownish overlay (`#3d2220`) — a single warm anomaly in a cool material system, signaling selection through material temperature rather than geometric distinction.
