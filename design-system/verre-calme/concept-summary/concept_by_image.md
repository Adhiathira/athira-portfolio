...
The full-bleed nature of the photograph, combined with the floating card pattern, creates a sense of **depth and dimensionality** in an otherwise static layout — the photograph recedes, the modal advances. The surface of the white modal card is not incidental: its matte flatness evokes cold-press paper, a material with a specific cultural register — printing, publication, couture lookbooks. That tactile reference grounds what could otherwise read as pure abstraction.
...
There is no section alternation visible. No background transitions, dividers, or color-shift separators. The photograph provides the visual depth and contrast.
...
The type system uses **Manrope** for all headings and structural text — a neutral geometric grotesque with a clean, unhurried rhythm — applied at precise tracking intervals that create tonal hierarchy through spacing rather than weight variation. All typographic variation is achieved through:

- Weight: 400 (body) vs 600–700 (headings)
- Scale: small nav → 15px body → 23px h2 → 46px h1
...
The visible interface uses only two functional colors: white and near-black. The absence of decorative accent color is a deliberate **visual authority signal** — design vocabulary communicated through restraint rather than richness.
