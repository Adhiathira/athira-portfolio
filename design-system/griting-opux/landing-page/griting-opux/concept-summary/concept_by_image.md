# Concept Summary — Griting Landing Page
...
Two CTAs share the frame: a solid green primary button ("Get in Touch") and an outline ghost button ("Learn More"). The pairing is standard but executed cleanly — the outline button uses a green border that ties it to the brand field without competing with the primary action.
...
## Opux Motion Supplement

*The following section describes how Opux's scroll-video behavior and visual interaction patterns translate into the griting-opux merged system. All color references from Opux's original dark-teal palette (#0d1a1f, #6b2a24, #9d5e5b) are remapped to Griting's green world (#003620, #00693e) throughout.*

### Motion as Infrastructure Signal

Opux's core motion strategy — alternating between animated, cinematic sections and calm, information-dense sections — maps naturally onto Griting's infrastructure narrative. Where Opux used a warm-to-cool gradient hero to front-load emotional energy, the griting-opux merged system uses the deep forest green (#003620) as the persistent atmospheric ground, with motion serving as depth indicator rather than temperature shift. The green environment does not spike and release; it breathes.

Opux's animated wireframe terrain (originally rendered in muted teal lines with a low-contrast opacity pass) is reinterpreted in the merged system as a **forest-floor topographic grid** — the same perspective-distorted plane, but rendered in deep green line strokes (#00693e at 15–20% opacity) against the #003620 field. The terrain does not suggest outer space or digital void; it suggests root systems, growth networks, the invisible infrastructure beneath a canopy. This is more grounded than Opux's original and more true to Griting's identity.

### Layered Depth in the Hero

Opux established a three-layer depth system for its hero: background animated terrain, midground decorative ghost-type, foreground editorial content. This layering behavior is carried into the griting-opux system, but with Griting's compositional grammar:

- **Background layer:** Forest-floor terrain grid animated in slow forward motion (Opux's parallax pace), rendered in #00693e at low opacity over #003620
- **Midground layer:** Large ghost-outline letterforms from Griting's typographic identity — the G-icon or "GRITING" at full viewport width, rendered at 4–6% white opacity as pure texture
- **Foreground layer:** Griting's bottom-left headline placement, preserved exactly. The editorial bottom-left anchor is Griting's typographic identity; Opux's centered stack is set aside.

The hero communicates the same depth Opux achieved, but grounds it in the forest rather than in space.

### Scroll-Video Behavior Remapped to Green

Opux's Media Showcase Section — in which a product video sits inside a browser-chrome container over the animated terrain background — is directly applicable to the griting-opux system. In the merged design, this pattern becomes the **GritMe Demonstration Beat**: a centered, contained presentation frame (browser chrome, #003620 surface, #00693e border accent) sitting over the reactivated terrain background. The terrain's return in this mid-page section serves the same purpose it did in Opux: it signals a narrative chapter shift, from analytical infrastructure to lived demonstration.

Video content within this frame follows Griting's proof-over-promise ethic. The video does not demonstrate a concept; it shows the system working. Color grading for any video assets produced for this section follows the green-world palette: cool shadows in #003620 range, midtone highlights lifted toward #00693e, warm skin tones preserved but shifted slightly cool to read as native to the environment.

### Alternating Pace: Opux's Pacing Grammar in Griting's Architecture

Opux's most transferable motion principle is its deliberate alternation between **animated narrative sections** (terrain active, cinematic) and **structured information sections** (terrain absent, static, content-dense). This pacing grammar slots directly into Griting's existing section architecture:

| Section | Opux Pacing Signal | Griting-Opux Behavior |
|---|---|---|
| Hero | Animated terrain, cinematic | Terrain active, bottom-left anchor, green field |
| New Infrastructure | Static, information-dense | Terrain absent, off-white decompression |
| Explore Ecosystem | Dark green gradient, transitional | Subtle terrain reintroduction at low opacity |
| GritMe Talent/Incubation | Static, evidence-first | Terrain absent, product mockup as primary media |
| GritMe Demo Beat | Animated terrain, cinematic | Terrain fully active, video-in-frame presentation |
| Testimonials | Static, human | Terrain absent, white background, avatar-forward |
| Footer | Dark green, closed | Terrain suggested at very low opacity as texture |

### Component Motion Vocabulary

Opux's component-level motion — fade-in with slide-up on scroll reveal, staggered card entrances within grid sections — is adopted in the merged system without alteration. These are implementation-level decisions that do not conflict with Griting's identity. The stagger timing follows Opux's observed rhythm: approximately 80–120ms per card offset, with a 300–400ms base duration for the fade-slide.

Opux's hover states on cards (subtle edge glow) translate to the griting-opux system as a **green glow**: a 0–8px box shadow in #00693e at 30–40% opacity, expanding on hover. This replaces Opux's cool-teal glow with Griting's chromatic signature without changing the interaction pattern.

### Shape Language Convergence

Opux used rounded rectangles consistently (8px for buttons, 16px for cards) with no pill shapes. Griting's original design is also non-pill, non-sharp. The merged system preserves this: 8px buttons, 16px cards, no border-radius extremes. Glassmorphism card surfaces from Opux are adapted to semi-transparent #003620 overlays (not dark-teal) where atmospheric card depth is needed — for instance, in the demo showcase section or any section where the terrain background is active.

### Ghost Letterforms as Brand Texture

Opux used decorative typographic ghost letterforms in the hero as midground texture — a high-design pattern from luxury editorial. This behavior is preserved in the merged system and becomes a Griting brand signal. The ghost forms are Griting's G-icon or the wordmark, not neutral letterforms. They carry the brand's etymology (the split G/riting form) as pure texture, visible only in the atmospheric sections where the terrain is active. In static, off-white sections, ghost letterforms do not appear — they are reserved for the green-world moments.

### Navigation Behavior

Opux's sticky transparent navigation (floating above the hero, no background surface) is compatible with Griting's minimal nav. In the merged system the nav is sticky and transparent over the hero green field — the G-icon and wordmark read white-on-#003620 without a surface. As the page scrolls into off-white sections, the nav acquires a #003620 background at approximately 95% opacity with a 1px bottom border in #00693e. This is Opux's adaptive transparency pattern recolored to Griting's palette.
