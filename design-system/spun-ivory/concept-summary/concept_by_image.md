...
The hero uses a centered single-column layout with generous vertical padding above and below the text block. The horizontal text block width is intentionally constrained — significantly narrower than the full viewport — which creates substantial lateral negative space. The headline sits tightly above the CTA with minimal inter-element gap; the micro-copy beneath maintains further compression. Spacing is not modular-dense — it follows an editorial breathing rhythm. Handled in the hand, the page would feel like a folded broadsheet — its proportions belong to print before they belong to a screen. The generous lateral margins behave like gutter rules, keeping content from touching the binding edge.
...
- **Legal/Copyright:** Far right, bottom row — "© 2026" in muted gray, minimal visual weight.
