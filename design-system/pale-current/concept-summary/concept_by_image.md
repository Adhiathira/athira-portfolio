...
Every component here carries physical presence — these are not abstract rectangles but surfaces with tactile logic. **Cards** are cast from a slightly warmer composite than the section ground (`#0f2128` vs `#0d1a1f`), the difference small enough to read as depth rather than contrast, as though the card surface were a different material lit from the same source. Separation comes through **background warmth and spacing**, not outline or shadow — the card simply occupies a distinct material register from the ground beneath it. `16px` border-radius softens the perimeter, suggesting fabricated edges over raw cuts.
