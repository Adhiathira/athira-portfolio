...
The navigation floats transparent at the top of the page — Cormorant Garamond wordmark at left, Barlow-set links at right with 0.08em letter-spacing. On scroll, the nav resolves: the parchment background fades in behind it (backdrop-filter blur), and a single 1px warm greige rule appears beneath — the thinnest possible structural signal. The scroll border does not appear until needed; at top of page there is no chrome, only content.
